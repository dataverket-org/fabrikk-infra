...
| `backup/` | Nothing in the cluster: `hov1/` is the home site's compose stack, versitygw with the posix backend behind the site's step-ca, the S3 endpoint the backups are written to. |
...
| `213.128.185.82:443` | CNPG's Barman Cloud plugin and restic, the backup writers | The home site's versitygw (`backup/hov1`), by address so no zone is in the backup path; TLS from the site's step-ca, its root pinned here |
